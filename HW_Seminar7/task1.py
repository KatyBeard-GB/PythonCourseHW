...
slovar = {'а', 'у', 'о', 'ы', 'и', 'э', 'я', 'ю', 'ё', 'е'}

poems = input('Введите стих: ').split()
beat = []
poem = list(map(lambda x: x.split('-'), poems))
for i in range(len(poem)):
    sumBeat = 0
    for k in range(len(poem[i])):
        sumBeat += len(list(filter(lambda x: x in slovar, poem[i][k])))
    beat.append(sumBeat)
if len(set(beat)) == 1: print('Парам пам-пам')
